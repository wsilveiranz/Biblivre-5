					...
					<div class="value">
						<select name="${key}">
							<c:forEach var="paragraphAlignment" items="<%= ParagraphAlignment.values() %>">
								<option value="${paragraphAlignment.string}"
									<c:out value="${value eq paragraphAlignment.string ? "selected" : ""}"/>
								>
									<i18n:text key="administration.configuration.label_print.${paragraphAlignment.string}"/>
								</option>
							</c:forEach>
						</select>
					</div>
